...
export const fetchEditionData = createAsyncThunk(
  ...
  (data: any) => {
    return EditionService?.getEditions1({
      filter: data.filter,
...
export const editEditionData = createAsyncThunk(
  ...
  ({ id, dTo }: { id: any; dTo: any }) => {
    return EditionService?.putEditions({ id: id, requestBody: dTo }).then(
      (result: any) => {
        return result;
      }
    );
...
export const getEditionsAll = createAsyncThunk("editions/all", async () => {
...
export const putEditionsMoveAllTenants = createAsyncThunk(
  ...
  async ({ id, editionId }: { id?: string; editionId?: string }) => {
...
export const saveFeaturesEdition = createAsyncThunk(
  ...
  (data: any) => {
    return FeaturesService.putFeatures({
      providerName: "E",
      providerKey: data.id,
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deleteEditionData.rejected, (state, action) => {
      ...
      state.error =
        ...
        "Unable to delete Edition, It is in use by tenants.";
      state.alert = true;
