...
import { RdsButton, RdsSearch } from "../../../../../bpmn-integration-elements/src";
...
import {
  ...
  getAllBlogPost,
  deleteBlogPosts,
  postBlogsPostsCreateAndSendToReview,
  postBlogPostsDraft,
  postBlogPostsPublish,
...
const BlogPost = () => {
  const blogListData = useAppSelector(
    (state) => state.persistedReducer.blogPost?.blogPost?.items
  ) as any;
  const BlogPostData = useAppSelector(
    (state) => state.persistedReducer.blogPost
  ) as any;
  ...
  useEffect(() => {
    const tempBlogPost = BlogPostData?.allBlogPosts?.items?.map((ele: any) => ({
      ...
      rowActions:
        ele.status == 1
          ? { id: "draft", displayName: t("CmsKit.Draft"), modalId: "draft" }
          : {
              id: "publish",
              displayName: t("CmsKit.Publish"),
              modalId: "publish",
            },
    }));
  ...
  const postDraftHandler = (data: any) => {
    dispatch(postBlogsPostsCreateAndSendToReview(data) as any).then(
      (res: any) => {
        dispatch(
          getAllBlogPost({
            skipCount: skipCount,
  ...
  const handlerEditPost = (data: any) => {
    dispatch(
      editBlogPostData({ id: tableDataRowid, postTypeDto: data }) as any
    ).then((res: any) => {
  ...
  const deleteHandler = () => {
    dispatch(deleteBlogPosts({ id: tableDataRowid }) as any).then(
      (res: any) => {
  ...
  const handlerPublish = () => {
    dispatch(postBlogPostsPublish({ id: tableDataRowid }) as any).then(
      (res: any) => {
  ...
  const handlerDraft = () => {
    dispatch(postBlogPostsDraft({ id: tableDataRowid }) as any).then(
      (res: any) => {
  ...
  const onSortHandler = (col: any, key: string) => {
    console.log("Blog Post data-table -", col, key);
            ...
            <div className="card px-4 py-4 border-0 rounded-0 card-full-stretch">
              <div className="row align-items-center">
                <div className="col-xxl-3 col-xl-3 col-lg-3 col-12 mb-2">
                  <RdsSearch placeholder={t("Search") || ""} size="small" />
                </div>
                <div className="col-xxl-9 col-xl-3 col-lg-3 col-12 mb-3 d-flex justify-content-xxl-end justify-content-xl-end justify-content-lg-end justify-content-md-end justify-content-start">
                  ...
                  <RdsCompAlertPopup
                    ...
                    messageAlert={
                      t("The selected Resource will be Deleted Permanently") ||
                      ""
                    }
                  ...
                  <RdsCompAlertPopup
                    alertID="publish"
                    messageAlert={
                      t("CmsKit.BlogPostPublishConfirmationMessage") || ""
                    }
                    alertConfirmation={t("AbpUi.AreYouSure") || ""}
                    deleteButtonLabel={t("AbpUi.Yes") || ""}
                  ...
                  <RdsCompAlertPopup
                    alertID="draft"
                    messageAlert={
                      t("CmsKit.BlogPostDraftConfirmationMessage") || ""
                    }
                    alertConfirmation={t("AbpUi.AreYouSure") || ""}
                    deleteButtonLabel={t("AbpUi.Yes") || ""}
