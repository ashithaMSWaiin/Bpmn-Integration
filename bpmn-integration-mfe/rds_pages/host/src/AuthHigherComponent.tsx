...
} from "../../../../bpmn-integration-react-core/src";
...
const AuthHigherComponent = (props: MainProps) => {
  ...
  // const [auth, auth] = useState(false);
  ...
  let configLocalizationCalled = false;
  ...
  const authVerification = async () => {
    ...
    if (
      ...
      // setauth(true);
      setAuth('true');
      if (!configLocalizationCalled) {
        await configLocalization();
        configLocalizationCalled = true;
      }
    }
  ...
  async function tokenRefresh() {
    ...
    const newAccessTokenExpiration = data.expires_in * 2; // Double the expiration time
    const newRefreshTokenExpiration = newAccessTokenExpiration * 2; // Double the expiration time of access token 

    localStorage.setItem("expiresIn", newAccessTokenExpiration.toString());
    localStorage.setItem("refreshTokenExpiration", newRefreshTokenExpiration.toString());

    // Check if both session timeout and token expiration have expired
    const currentTime = new Date();
    const expirationFinalTime = new Date(currentTime.getTime() + newAccessTokenExpiration * 1000);
    if (currentTime > expirationFinalTime) {
      // If both session timeout and token expiration have expired, navigate to the home page
      navigate("/");
    }
    return data;
  }
  ...
  function handleSpecialPaths(currentPath: string, i18n: any): MenuItem[] {
    ...
    } else if (currentPath === "/linked-accounts") {
      document.title = "Linked Accounts";
      const navtitl = i18n.t("AbpAccount.LinkedAccounts");
      setCurrentTitle(navtitl);
      return [
        {
          key: "special-linked-accounts",
          label: navtitl,
          path: "/linked-accounts",
          icon: "",
        },
      ];
    } else if (currentPath === "/authority-delegation") {
      document.title = "Authority Delegation";
      const navtitl = i18n.t("AbpAccount.AuthorityDelegation");
      setCurrentTitle(navtitl);
      return [
        {
          key: "special-authority-delegation",
          label: navtitl,
          path: "/authority-delegation",
          icon: "",
        },
      ];
  ...
  const removeLocalStorage = async () => {
    ...
    localStorage.removeItem("activeMenuNodeKey"); 
    localStorage.removeItem("activeMenuParentKey");   
  ...
  useEffect(() => {
    ...
    updateLocalStorage();
    handleDataHost();
  }, [location.pathname, currLanguage, RolesPermission, currTheme, isImpersonation, dataHost]);
