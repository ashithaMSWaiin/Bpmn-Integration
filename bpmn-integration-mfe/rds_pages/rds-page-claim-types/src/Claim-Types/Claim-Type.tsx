...
import { RdsOffcanvas, RdsButton, RdsAlert, RdsSearch } from "../../../rds-elements";
...
import {
  deleteClaimTypesRequest,
  getClaimTypes1Request,
  getClaimTypesRequest,
  getRolesAllClaimTypesRequest,
  postClaimTypesRequest,
  putClaimTypesRequest,
...
const ClaimType = () => {
  ...
  }, [dispatch, skipCount, debouncedSearchTerm, maxResultCount]);
  ...
  const DeleteHandler = (e: any) => {
    dispatch(deleteClaimTypesRequest({ id: tableDataRowid }) as any).then(
      (res: any) => {
  ...
  const submitHandler = (data: any) => {
    dispatch(postClaimTypesRequest({ requestBody: data }) as any).then(
      (res: any) => {
  ...
  const onEditHandler = (editClaimData: any) => {
    const dTo = {
      id: claimTypesId,
      requestBody: editClaimData,
      name: val,
    };
    dispatch(putClaimTypesRequest(dTo) as any).then((res: any) => {
  ...
  const onSortHandler = (col: any, key: string) => {
    console.log("Claim Type data-table -", col, key);
