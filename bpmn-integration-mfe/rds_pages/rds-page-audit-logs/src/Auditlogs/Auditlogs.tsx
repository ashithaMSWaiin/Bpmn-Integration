...
const Auditlogs = (props: RdsPageAuditlogsProps) => {
  ...
  const entityNavtabsItems = [
    {
      label: t("AbpAuditLogging.ChangeDetails"),
      tablink: " #nav-change_history",
      id: "0",
    },
    {
      label: t("AbpAuditLogging.FullChangeHistory"),
      tablink: " #nav-full_history",
      id: "1",
    },
  ];
  ...
  const pageNavtabsItems = [
    {
      label: t("AbpAuditLogging.AuditLogs"),
      tablink: " #nav-audit-log",
      id: "0",
    },
    {
      label: t("AbpAuditLogging.EntityChanges"),
      tablink: " #nav-entity-changes",
      id: "1",
    },
  ];
  ...
  }, [dispatch, skipCount, maxResultCount, activePageId, sorting]);

  useEffect(() => {
    if (audituser?.getAuditLogs?.items) {
      const auditDataTable = audituser.getAuditLogs.items.map(
        (dataAudit: any) => {
          let timeCol = null;
          if (dataAudit.executionTime === "2023-08-25T09:03:26.8597775") {
            timeCol = <span>Hello</span>;
          }
          let ipAddress = "";
    ...
    setTotalRecords({
      ...totalRecords,
      audit: audituser?.getAuditLogs?.totalCount,
    });
  }, [audituser?.getAuditLogs]);

  useEffect(() => {
    if (audituser?.getAuditLogsEntityChanges?.items) {
      const entityDataTable = audituser.getAuditLogsEntityChanges.items.map(
        (data: any) => {
    ...
    setTotalRecords({
      ...totalRecords,
      entity: audituser?.getAuditLogsEntityChanges?.totalCount,
    });
  ...
  useEffect(() => {
    if (audituser?.getAuditLogsEntityChangeWithUsername?.items) {
      const changeHistoryData =
        audituser.getAuditLogsEntityChangeWithUsername?.items.map(
          (item: any) => {
            return {
              id: item.id,
  ...
  useEffect(() => {
    if (audituser?.getAuditLogsEntityChangesWithUsername?.items) {
      const fullChangeHistoryData =
        audituser.getAuditLogsEntityChangesWithUsername.items.map(
          (item: any) => {
            return {
              id: item.id,
              propertyTypeFullName: item.propertyTypeFullName,
  ...
  const onEntityActionSelection = (rowData: any, actionId: any) => {
    ...
    dispatch(
      getAuditLogsEntityChangesWithUsernameRequest({
        ...
        entityTypeFullName: rowData.entityTypeFullName,
      }) as any
    );
  };

  const auditPagePaginationHandler = (
    currentPage: number,
    recordsPP: number
  ) => {
  ...
  const entityPagepaginationHandler = (
    currentPage: number,
    recordsPP: number
  ) => {
    const skipCount = recordsPP * (currentPage - 1);
  ...
  const onHttpCodeFilter = (event: any, val: string) => {
    ...
    dispatch(
      getAuditLogsRequest({ ...searchFilter, httpStatusCode: val }) as any
    );
  };
  ...
  const onEntityFullNameFilter = (event: any) => {
    ...
    if (event.target.value.length > 4) {
      dispatch(
        getAuditLogsEntityChangesRequest(searchEntityChangesFilter) as any
      );
    } else {
      dispatch(
        getAuditLogsEntityChangesRequest({
  ...
  const onSortHandler = (col: any, key: string) => {
    console.log("auditLogs data-table -", col, key);
      ...
      <div className="row mt-3">
        ...
        <div className="col-md-9 mb-3">
          <RdsBadge
            ...
            colorVariant={getColorVariantForBadge(
              selectedRowData.selectedRowData[0]?.httpStatusCode
            )}
          ></RdsBadge>
        </div>
        ...
        <div className="col-md-9 mb-3">
          <RdsBadge
            ...
            colorVariant={getColorVariantForBadge(
              selectedRowData.selectedRowData[0]?.httpMethod
            )}
          ></RdsBadge>
        </div>
        ...
        <div className="col-md-9 mb-3">
          <span>
            {selectedRowData.selectedRowData[0]?.url.props.children[2]}
          </span>
        </div>
