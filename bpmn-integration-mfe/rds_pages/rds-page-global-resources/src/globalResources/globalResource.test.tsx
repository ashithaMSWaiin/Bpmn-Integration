...
describe("Chats page", () => {
    it("renders Chats page correctly", async () => {
        ...
        document.querySelector('[label="CmsKit.SaveChanges"]');

    });

    it("Check Global Resource page function behaviour", async () => {
        const mockDispatch = jest.fn();
        (useAppDispatch as jest.Mock).mockReturnValue(mockDispatch);
        ...
        const Global = render(
            <Provider store={store}>
                <GlobalResources />
                <RdsNavtabs navtabsItems={[]} type={"tabs"} />
                <RdsAlert alertmessage={""} colorVariant={""} />
                <RdsCompCodeMirror code={""} />
                <RdsButton />
            </Provider>
        );
