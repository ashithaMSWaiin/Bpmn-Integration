...
const TextTemplate = () => {
  ...
  // Use Effects
  useEffect(() => {
    const IsEdit = isgrantedpolicies(
    ...
    if (IsEdit) {
      resultArray.push({
        id: "edit",
        displayName: t("TextTemplateManagement.EditContents"),
        offId: "text-temp-edit-off",
      });
      resultArray.push({
        id: "customizePerCulture",
        displayName: t("TextTemplateManagement.CustomizePerCulture"),
        offId: "cust-per-cult",
      });
    }
  ...
  useEffect(() => {
    if (textTemplate?.languages?.localization?.languages) {
      const tempLanguage =
        textTemplate?.languages?.localization?.languages?.map((item: any) => {
          return {
  ...
  function onActionSelection(rowData: any, actionId: any) {
    ...
    if (actionId == "customizePerCulture") {
      setSelectedRefValue(
        selectedRefValue === "en-GB" ? "English (UK)" : selectedRefValue
      );
      setSelectedTargetCulture(
        selectedTargetCulture === "ar" ? "العربية" : selectedTargetCulture
      );
  ...
  const callApi = async (data: any) => {
    await dispatch(
      getTemplateContentTar({ template: data, culture: refTar.tar }) as any
    );
    await dispatch(
      getTemplateContentRef({ template: data, culture: refTar.ref }) as any
    );
  };
  ...
  function handleRefChange(value: any, cultureType: string, stateKey: string) {
    if (cultureType === "ref") {
      ...
      dispatch(
        getTemplateContentRef({ template: displayName, culture: value }) as any
      );
    } else if (cultureType === "target") {
      setSelectedTargetCulture(value);
      dispatch(
        getTemplateContentTar({ template: displayName, culture: value }) as any
      );
    }
  }
  ...
  const onSortHandler = (col: any, key: string) => {
    console.log("Text-template data-table -", col, key);
          ...
          <div className="px-2">
            <div className="form-group mb-3">
              <div className="pe-2">
                <RdsLabel
                  fontWeight="normal"
                  label={t("TextTemplateManagement.Name") || ""}
                ></RdsLabel>
              </div>
            ...
            <div className="form-group pb-4 fw-bold">
              <label className="form-label fw-normal mb-0">
                {" "}
                {t("TextTemplateManagement.TemplateContent") || ""}
              </label>
