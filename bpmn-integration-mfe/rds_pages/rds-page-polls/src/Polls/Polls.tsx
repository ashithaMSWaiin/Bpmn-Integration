...
import {
  ...
  postPollRequest,
  putPollRequest,
} from "../../../../libs/state-management/poll-admin/poll-admin-slice";
...
const Polls = (props: any) => {
  ...
  const actions = [
    { id: "edit", displayName: t("AbpUi.Edit"), offId: "poll-edit-off" },
    {
      ...
      modalId: "poll-delete-off",
    },
    {
      id: "result",
      displayName: t("CmsKit.ShowResults"),
  ...
  const isFormValid =
    isQuestionValid(questionData.question) && isCodeValid(questionData.code);
  ...
  const editDataHandler = () => {
    const removeIdsFromPollsOptions = editPollsOptionData.map(
    ...
    const data = {
      ...editQuestionData,
      pollOptions: removeIdsFromPollsOptions,
    };
  ...
  function handleAddNewPoll() {
    ...
    dispatch(postPollRequest({ requestBody: allData }) as any).then(
      (res: any) => {
        if (res.type.includes("rejected")) {
          setAlert({
            ...Alert,
  ...
  const onSortHandler = (col: any, key: string) => {
    console.log("Polls data-table -", col, key);
          ...
          <div className="col-md-12">
            <div className="card px-4 py-4 border-0 rounded-0 card-full-stretch">
              ...
              >
                ...
                >
                  ...
                  {activeNavTabIdEdit == "1" && (
                    <>
                      <RdsCompPollsOption
                        optionsData={editPollsOptionData}
                        getPollsOptionData={getPollsEditOptionData}
                      ></RdsCompPollsOption>
                    </>
                  )}
                </RdsNavtabs>
              ...
              <RdsOffcanvas
                ...
                preventEscapeKey={false}
              >
                {pollsResultData.length &&
                  pollsResultData.map((e: any) => (
                    <>
