...
const Language = () => {
  ...
  const [sorting, setSorting] = useState<string>("creationTime desc");
  ...
  const actions = [
    { id: "edit", displayName: t("AbpUi.Edit"), offId: "language-edit-off" },
    {
      ...
      modalId: "language-delete-off",
    },
    {
      id: "setAsDefaultLanguage",
      displayName: t("LanguageManagement.SetAsDefaultLanguage"),
    },
  ];
  ...
  const finalArray = flagListName.map((name, index) => {
    let value =
      flagListCode && flagListCode?.length > index ? flagListCode[index] : null;
    return {
      option: name,
      value: value,
    };
  ...
  }, [dispatch, skipCount, maxResultCount, debouncedSearchTerm, sorting]);
  ...
  useEffect(() => {
            ...
            <>
              {item.isDefaultLanguage ? (
                <span className=" d-flex align-items-center ">
                  <RdsLabel label={item.displayName} class="me-1"></RdsLabel>
                  <RdsBadge
                    label={"Default"}
                    size={"small"}
                    badgeType={"rectangle"}
    ...
    if (data?.getLanguagesCultureList[1]) {
      const filteredData = data.getLanguagesCultureList
      .filter((items: any) => items.displayName !== "Invariant Language (Invariant Country)")
      const tempName = filteredData.map((items: any) => {
        return {
  ...
  const onSaveHandler = (data: {
    ...
    dispatch(postLanguagesRequest({ requestBody: data }) as any).then(
      (res: any) => {
  ...
  const getCountryName = (flagIcon: any) => {
    const countryName = flagListName.find(
      (item, index) => flagListCode[index] === flagIcon
    );
    return countryName || "";
  };
  ...
  const onActionSelection = (rowData: any, actionId: any) => {
    ...
    if (actionId == "setAsDefaultLanguage") {
      dispatch(putLanguagesSetAsDefaultRequest({ id: rowData.id }) as any).then(
        (res: any) => {
          dispatch(
            getLanguagesRequest({
              skipCount: skipCount,
  ...
  const onDeleteHandler = () => {
    dispatch(deleteLanguagesRequest({ id: tableDataRowid }) as any).then(
      (res: any) => {
        dispatch(
          getLanguagesRequest({
            skipCount: skipCount,
                    ...
                    <div className="col-md-6 mb-3">
                      <div className="form-group">
                        <RdsSelectList
                          ...
                          onChange={(item: any) =>
                            setSelectedFlagIcon(item.value)
                          }
                        ></RdsSelectList>
                      </div>
                    </div>
