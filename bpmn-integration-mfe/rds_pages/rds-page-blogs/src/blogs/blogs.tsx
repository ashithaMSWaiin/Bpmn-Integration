...
const Blogs = () => {
  ...
  const addDataHandler = () => {
    dispatch(addBlogsData({ name: nameInput, slug: slugInput }) as any).then(
      (res: any) => {
  ...
  const editDataHandler = (editBlogBody: any) => {
    ...
    dispatch(
      editBlogsData({ id: tableDataRowid, requestBody: editBlog }) as any
    ).then((res: any) => {
  ...
  const onSortHandler = (col: any, key: string) => {
    console.log("Blogs data-table -", col, key);
