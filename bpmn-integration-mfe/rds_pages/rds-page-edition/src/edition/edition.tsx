...
const Edition = (props: RdsPageEditionProps) => {
  ...
  useEffect(() => {
    ...
    if (IsDelete) {
      resultArray.push({
        ...
        modalId: "edition-delete-offc",
      });
    }

    resultArray.push({
      id: "move_all_tenant",
      displayName: t("Move all tenants"),
  ...
  useEffect(() => {
    if (editionuser?.featureIdentitySettings) {
      const sample = editionuser.featureIdentitySettings.groups.map(
        (x: any) => {
          return {
            name: x.name,
  ...
  function restoreFeatures() {
    dispatch(restoreToDefaultFeaturesEdition(tableDataRowid) as any).then(
      () => {
        dispatch(fetchFeaturesEdition(tableDataRowid) as any).then(
  ...
  const onSortHandler = (col: any, key: string) => {
    console.log("edition data-table -", col, key);
  ...
  const moveAllTenant = () => {
    ...
    dispatch(
      putEditionsMoveAllTenants({
        id: moveTenantData.tenantId,
        editionId: moveTenantData.editionId,
      }) as any
    ).then((res: any) => {
      dispatch(
        fetchEditionData({
    ...
    <div className="container-fluid p-0 m-0 h-100">
      <div className="card px-4 py-4 border-0 rounded-0 card-full-stretch">
        ...
        >
          <div className="pb-3">
            <span>
              Move all tenants with <b>{editionName}</b> edition to:
            </span>
          </div>
          {showSelectlist && (
            <div className="">
          ...
          {!showSelectlist && (
            <div className="pb-3">
              <span>
                {" "}
                <b>This edition has no tenants to move.</b>
              </span>
            </div>
          )}
          <div className="d-flex pb-3 flex-column-reverse flex-lg-row flex-md-column-reverse flex-xl-row flex-xxl-row footer-buttons gap-2">
