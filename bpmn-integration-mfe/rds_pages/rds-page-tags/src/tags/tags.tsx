...
import {
  ...
  postTagsRequest,
  putTagsRequest,
} from "../../../../libs/state-management/tag-admin/tag-admin-slice";
...
const Tags = () => {
  ...
  useEffect(() => {
    if (tags?.getTags?.items) {
      const tempData = tags.getTags?.items.map((item: any) => {
        return {
          id: item.id,
    ...
    setTotalRecords(tags?.getTags?.totalCount);
  }, [tags?.getTags]);
  ...
  const editTags = (event: any, data: any) => {
    ...
    dispatch(
      putTagsRequest({ id: tableDataRowid, requestBody: editBody })
    ).then((res: any) => {
  ...
  const saveUpdateTags = (tagsObj: any, event: any) => {
    ...
    dispatch(postTagsRequest({ requestBody: payload })).then((res: any) => {
  ...
  const onSortHandler = (col: any, key: string) => {
    console.log("Tags data-table -", col, key);
