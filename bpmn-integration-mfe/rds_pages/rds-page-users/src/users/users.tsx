...
const Users = (props: any) => {
  ...
  const [isLockoutEndData, setIsLockoutEndData] = useState<{
    id: string;
    lockoutEnd: string;
  }>();
  ...
  const organizationData = useAppSelector(
    (state) => state.persistedReducer.organizationUnit
  );
  ...
  const handlerClaim = () => {
    dispatch(
      putUsersClaimsRequest({ id: userId, requestBody: claimsTable }) as any
    );
  ...
  const createNewClaim = (event: any) => {
    ...
    let newTempData = allClaimsArray.filter(
      (data: any) => data.id === selectedData.claimType
    );
    if (selectedData.claimType && selectedData.claimValue) {
      ...
      setSelectedData({
        ...
        setInputReset(!inputReset); // Toggle the reset state
    }
  };
  ...
  const isFormValid =
    isPasswordValid(getUser.password) &&
    isEmailValid(getUser.email) &&
    isNameValid(getUser.name) &&
    isSurnameValid(getUser.surname) &&
    isUsernameValid(getUser.userName) &&
    isPhoneNumberValid(getUser.phoneNumber);
  ...
  useEffect(() => {
    if (
      ...
      Array.isArray(data.getUsersAllClaimTypes)
    ) {
  ...
  useEffect(() => {
    ...
    const IsChangePermissions = isgrantedpolicies(
      "AbpIdentity.Users.ManagePermissions",
      configData
    );
    const IsImpersonated = isgrantedpolicies(
    ...
    resultArray.push({
      id: "claims_off",
      displayName: t("AbpIdentity.Claims"),
      offId: "claims-off",
    });
    if (IsChangePermissions) {
      resultArray.push({
        id: "permission_off",
        displayName: t("AbpPermissionManagement.Permissions"),
        offId: "permission-off",
      });
    }
    resultArray.push({
      id: "set_password",
      displayName: t("AbpIdentity.SetPassword"),
      offId: "set_password",
    });
    resultArray.push({
      id: "two_Factor_off",
      displayName: t("AbpIdentity.TwoFactor"),
      offId: "two-Factor-off",
    });
    if (IsImpersonated) {
      resultArray.push({
        id: "login_as_user",
        displayName: t("AbpIdentity.LoginWithThisUser"),
      });
    }
  ...
  useEffect(() => {
    dispatch(
      getUsers1Request({
        ...
        organizationUnitId: search.organizationUnitId,
        isLockedOut: search.isLockedOut,
        notActive: search.notActive,
        // sorting: sorting,
      }) as any
    );
  }, [
  ...
  useEffect(() => {
    if (data?.getUsers1?.items) {
      ...
      const tempData = data?.getUsers1?.items?.map((item: any) => {
        const isDelete = isgrantedpolicies(
        ...
        if (item?.creatorId !== null && isDelete) {
          return {
            id: item.id,
            userName:
              item.lockoutEnd !== null && item.lockoutEnd !== undefined ? (
                <div>
                  <RdsIcon
                    name="lock"
                    height="15px"
            ...
            rowActions: {
              ...
              modalId: "user-delete-off",
            },
            rowActionsAdd:
              item.lockoutEnd == null
                ? { id: "lock_off", displayName: t("Lock"), offId: "lock-off" }
                : { id: "unlock_user", displayName: t("Unlock") },
          };
        } else {
          return {
            id: item.id,
  ...
  //================ Normal variables and constants
  ...
  function onActionSelection(rowData: any, actionId: any) {
    ...
    if (actionId == "login_as_user") {
      ...
      userImpersonationService(dto).then(async (res: any) => {
        if (res.access_token) {
    ...
    if (actionId == "lock_off") {
      dispatch(getUsersRequest(rowData) as any).then((res: any) => {
        const formattedLockoutEnd = dateFormateConvertLockout(
          rowData.lockoutEnd
        );
        setIsLockoutEndData({
          ...isLockoutEndData,
    ...
    if (actionId === "claims_off") {
      ...
      setClaimsTableData([]); // Clear the claims table data
      dispatch(getUsersAllClaimTypesRequest() as any);
      dispatch(getUsersClaimsRequest({ id: rowData.id }) as any).then(
        (res: any) => {
          if (getUsersClaimsRequest.fulfilled.match(res)) {
    ...
    if (actionId === "permission_off") {
      if (
        permissionData.getPermissions &&
        permissionData.getPermissions?.groups?.length > 0 &&
        permissionData.getPermissions?.groups == userPermission
      ) {
        setUserPermission(permissionData.getPermissions?.groups);
      }
      dispatch(
  ...
  function createNewUser(data: any) {
    ...
    dispatch(postUsersRequest({ requestBody: tempData }) as any).then(
      (res: any) => {
  ...
  function offcanvasClose() {
    ...
    if (passwordGenerated) {
      // setPassword(""); // Clear the password state only if it was auto-generated
      setPasswordGenerated(false); // Reset the flag
    }
    setUserData({
      name: "",
      surname: "",
  ...
  function handleUserUpdate(data: any) {
    ...
    dispatch(
      putUsersRequest({ id: userId, requestBody: updateData }) as any
    ).then((res: any) => {
  ...
  const handlerSetPassword = (data: any) => {
    const tempPasswordData = {
      id: userId,
      requestBody: { newPassword: data },
    };
    dispatch(putUsersChangePasswordRequest(tempPasswordData) as any).then(
      (res: any) => {
  ...
  const onSortHandler = (col: any, key: string) => {
    console.log("Users data-table -", col, key);
        ...
        <div className="col-md-12">
          <div className="card px-4 py-4 border-0 rounded-0 card-full-stretch users">
            <div className="row">
              <div className="col-xxl-7 col-xl-7 col-lg-12 col-md-12 col-12">
                <div className="row">
                  <div className="col-xxl-4 col-xl-3 col-md-4 mb-md-3 mb-2">
                    <RdsSearch
                      key={filterKey}
                      value={search.filter}
                ...
                <div className="align-items-center d-block d-lg-flex d-md-flex d-xl-flex d-xxl-flex justify-content-between">
                  <div className="d-flex gap-3 mb-3 mb-md-0">
                    <RdsCheckbox
                      ...
                      onChange={(e: any) => {
                        setSearch({ ...search, isLockedOut: e.target.checked });
                      }}
                    ></RdsCheckbox>
                    <RdsCheckbox
                      label={t("Not Active") || ""}
                      ...
                      onChange={(e: any) => {
                        setSearch({ ...search, notActive: e.target.checked });
                      }}
                    ></RdsCheckbox>
                    <a
          ...
          >
            <RdsCompSetPassword
              ...
              password={function (password: any): [any, any] {
              ...
              <div>
                <div className="form">
                  <div className="row">
                    ...
                    <div className="col-md-2 mt-xxl-2 mt-xl-1 mt-lg-1 mt-md-1 ps-xxl-1 ps-xl-1 ps-lg-1 ps-md-1 pt-xxl-4 pt-xl-4 pt-lg-4 pt-md-4">
                      <RdsButton
                        type={"button"}
                        label={t("") || ""}
                        icon="plus"
                ...
                <div className="form-group">
                  <>
                    <RdsCheckbox
                      ...
                      onChange={(e: any) => {
                        setIsEnabledData(e.target.checked);
                      }}
