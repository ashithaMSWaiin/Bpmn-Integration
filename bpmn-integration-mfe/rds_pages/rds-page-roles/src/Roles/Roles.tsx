...
import { isgrantedpolicies } from "../../../../../bpmn-integration-react-core/src/index";
...
const Roles = () => {
  ...
  const currentLanguage = useAppSelector(
    (state) => state.persistedReducer.host?.currentLanguage
  );
  //**************  useState Hook  ***************************
  ...
  const [saveClaim, setSaveClaim] = useState<
    { claimType: any; claimValue: any; roleId: any; id: any }[]
  >([]);
  ...
  const handlerActions = (rowData: any, actionId: any) => {
    ...
    if (actionId === "permissions") {
      dispatch(
        getPermissionsRequest({
          providerName: "R",
          providerKey: rowData.provideKey,
        }) as any
      ).then((res: any) => {
        if (res.type === "permissions/getPermissionsRequest/fulfilled") {
          if (
            Data.roles?.permission &&
            Data.roles.permission?.groups?.length > 0 &&
            Data.roles?.permission?.groups == permissionListData
          ) {
    ...
    if (actionId === "claim") {
      dispatch(getRolesAllClaimTypesRequest() as any);
      dispatch(getRolesClaimsRequest({ id: rowData.id }) as any).then(
        (res: any) => {
  ...
  const handlerEditRole = () => {
    ...
    dispatch(putRolesRequest({ id: id, requestBody: dto }) as any).then(
      (res: any) => {
  ...
  const handlerPermission = () => {
    ...
    dispatch(
      putPermissionsRequest({
        providerName: "R",
        providerKey: permissions.providerKey,
        requestBody: permissions.requestBody,
      }) as any
    ).then((res: any) => {
  ...
  const onSortHandler = (col: any, key: string) => {
    console.log("Roles data-table -", col, key);
            ...
            <div className="pt-2 overflow-x-hidden overflow-y-scroll offcanvas-custom-scroll">
              <div className="pt-2">
                <RdsCompClaims
                  ...
                  id={id}
                  getEditClaimData={claimDataHandler}
                ></RdsCompClaims>
              </div>
              <div className="pb-3 d-flex flex-column-reverse flex-lg-row flex-md-column-reverse flex-xl-row flex-xxl-row flex-row gap-2 footer-buttons">
                <RdsButton
