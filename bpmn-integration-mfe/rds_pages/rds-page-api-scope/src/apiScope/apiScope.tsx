...
import {
  ...
  postScopesRequest,
  putScopesRequest,
} from "../../../../libs/state-management/scopes/scopes-slice";
...
const ApiScope = () => {
  ...
  useEffect(() => {
    ...
    const IsHistory = isgrantedpolicies(
    ...
    if (IsHistory) {
      resultArray.push({
        id: "history",
        displayName: t("AbpOpenIddict.ChangeHistory"),
  ...
  const handlerDeleteScope = () => {
    dispatch(deleteScopesRequest({ id: tableDataid }) as any).then(
      (res: any) => {
  ...
  const handlerAddScope = (data: any) => {
    ...
    dispatch(postScopesRequest({ requestBody: newDto }) as any).then(
      (res: any) => {
