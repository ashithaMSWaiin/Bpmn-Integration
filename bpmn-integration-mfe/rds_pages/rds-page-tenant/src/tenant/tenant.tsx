...
const Tenant = () => {
  ...
  }, [skipCount, maxResultCount, filter, sorting]);
  useEffect(() => {
  ...
  const handlerSetPassword = (data: any) => {
    ...
    dispatch(putTenantsSetPasswordRequest(tempPasswordData) as any).then(
      (res: any) => {
        if (res.type == "tenant/putTenantsSetPasswordRequest/rejected") {
          setAlert({
            ...Alert,
            show: true,
            message: t("Your request has been denied"),
  ...
  const handleActionSelection = (rowData: any, actionId: any) => {
    ...
    if (actionId === "login") {
      ...
      tenantImpersonationService(dto).then(async (res: any) => {
        if (res.access_token) {
  ...
  const onSortHandler = (col: any, key: string) => {
    console.log("Tenants data-table -", col, key);
                      ...
                      <div className="row align-items-center">
                        ...
                        <div className="col-xxl-4 col-xl-4 col-lg-4 col-md-4 col-12 mb-3">
                          <RdsDropdownList
                            ...
                            isPlaceholder={true}
                            onClick={(event, selectedCode) =>
                              onHttpCodeFilter(selectedCode)
                            }
                          />
                        </div>
                        <div className="col-xxl-4 col-xl-4 col-lg-4 col-md-4 col-12 mb-3">
                          <RdsDropdownList
                            ...
                            isIconPlaceholder={false}
                            isPlaceholder={true}
                            onClick={(event, selectedStatus) =>
                              onActivationStateFilter(selectedStatus)
                            }
                          />
                        </div>
                      </div>
