...
const Login = (_props: LoginProps) => {
  ...
  const [_currentLanguage, _setCurrentLanguage] = useState(
  ...
  useEffect(() => {
    if (loginData?.invalidCredential?.invalid == true) {
      setAlert({
        ...Alert,
        message:
          loginData.invalidCredential?.message ||
  ...
  const [validateTenantName, setValidateTenantName] = useState(
    "AbpUiMultiTenancy.NotSelected"
  );
  function validateTenant(data: any) {
    if (data) {
      validateTenantByName(data)
        .then((res: any) => {
  ...
  const configLocalization = () => {
    configurationService(currLanguage).then(async (res: any) => {
      ...
      localizationService(res.localization.currentCulture.cultureName).then(
        (resp: any) => {
          let data2 = {};
                  ...
                  <div>
                    <div className="login-border-bottom mb-4 pb-1 raaghu-logo">
                      <div className="text-center">
                        <img
                          src="./assets/raaghu_text_logo.svg"
                          height={"68px"}
                          width={"161px"}
                        ></img>
                      </div>
