...
import { RdsOffcanvas, RdsDatePicker, RdsInput } from "../../../rds-elements";
import {
  useAppDispatch,
  useAppSelector,
...
const PaymentRequests = () => {
  ...
  const actions = [
    {
      id: "information",
      displayName: t("Payment.Products"),
  ...
  useEffect(() => {
    if (
      !Array?.isArray(paymentRequests?.allPaymentRequests?.items) ||
      paymentRequests?.allPaymentRequests?.items?.length === 0
    ) {
