...
const RdsDropdownList = (props: RdsDropdownListProps) => {
  ...
  const customClass = props.className ? "form-control-sm " + fieldSize : "";
  useEffect(() => {
    setIsTouch(false);
    setCheckedCategoryList([]);
  }, [props.reset]);
      ...
      <div className={`dropdown ${block ? "w-100 mt-1" : ""}`} ref={dropdownRef}>
        {props.tooltip ? (
          <Tooltip text={props.tooltipTitle} place={props.tooltipPlacement}>
            <span
              className={`gap-2 ${offset} ${border} ${customClass}`}
              role="button"
            ...
            >
              <div
                className={
                  "d-flex align-items-center justify-content-xxl-between justify-content-xl-between justify-content-lg-between justify-content-md-between justify-content-center"
                }
              >
                {/* simple dropdown  */}
                {isTouch !== true &&
