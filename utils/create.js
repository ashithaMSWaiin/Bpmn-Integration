...
const portFilePath = path.join(
    __dirname,
    "../bpmn-integration-mfe/rds_pages/",
    "port-config.ts"
);
...
if (eTc == "core") {
    var mfeFolderPath = path.join(__dirname, "..", "bpmn-integration-mfe");
    execSync("npm install --save bpmn-integration-react-core", {
        cwd: ".",
        stdio: "inherit",
    });
    execSync("npm install --save bpmn-integration-react-core", {
    ...
    console.log("\x1b[32m%s\x1b[0m", "bpmn-integration-react-core successfully installed!!");
    process.exit(0);
}
...
let appFolderPath = ".";
if (eTc == "e") {
    appFolderPath = path.join(__dirname, "..", "bpmn-integration-elements");
} else if (eTc == "c") {
    appFolderPath = path.join(__dirname, "..", "bpmn-integration-components");
} else if (eTc == "p") {
    name = process.env.npm_config_pageName;
    appFolderPath = path.join(__dirname, "..", "bpmn-integration-mfe", "rds_pages");
} else if (eTc == "m") {
    name = process.env.npm_config_pageName;
    appFolderPath = path.join(__dirname, "..", "bpmn-integration-mfe", "rds_pages");
}
...
if (fs.existsSync(appFolderPath)) {
    ...
    // fs.writeFileSync(templateWebpackfile, updatedWebpackConfig);
        ...
        const filePathForPageComponent =
            "bpmn-integration-mfe/rds_pages/host/src/PageComponent.ts";
        ...
        let sideNavItemPath = path.join(__dirname, "../bpmn-integration-mfe/libs/main-menu");
        try {
            fs.writeFileSync(
        ...
        const mfeConfigFilePath = path.resolve(
            __dirname,
            "../bpmn-integration-mfe/rds_pages/mfe-config.ts"
        );
        ...
        const portConfigFilePath = path.resolve(
            __dirname,
            "../bpmn-integration-mfe/rds_pages/port-config.ts"
        );
        ...
        fs.readFile(
            "bpmn-integration-mfe/rds_pages/host/src/remote.d.ts",
            "utf8",
            (err, data) => {
                if (err) {
                    console.error(err);
        ...
        const hostWebpackpath = "bpmn-integration-mfe/rds_pages/host/webpack.config.js";
        ...
        const privateRouteAuthpath = "bpmn-integration-mfe/rds_pages/host/src/PrivateRoute_Auth.tsx";
        ...
        const packageFile = "bpmn-integration-mfe/package.json";
