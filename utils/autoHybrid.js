...
function convertToDisplayCase(str) {
...
export interface Rds${convertToPascalCase(`${componentName}`)}Props {
  ${propInfoForGeneration.map((element) => element.name + `${pageNameSingular}Prop?:any;`).join("\n  ")}
  ${navPropInfoForGeneration.map((element) => element.DisplayName + `${element.EntityName}Prop?:any;`).join("\n  ")}
  create?:boolean;
  update?:boolean;
  onSaveHandler?: any;
  offCanvasType?:any;
...
${navPropInfoForGeneration.map((element) => `${element.DisplayName}${element.EntityName}ListProp?:any;`).join("\n  ")}
...
const {
  ${propInfoForGeneration.map((element) => element.name + `${pageNameSingular}Prop`).join(",\n")},
  ${propInfoForGeneration
    .filter((element) => element?.EnumValues)
    .map((prop) => {
      return `${prop.name}EnumItems,`;
    })}
    ${navPropInfoForGeneration.map((element) => element.DisplayName + `${element.EntityName}Prop,`).join("\n")}
    ${navPropInfoForGeneration.map((element) => `${element.DisplayName}${element.EntityName}ListProp,`).join(",\n")}
  offCanvasType, 
  onSaveHandler,
  ${navConnectionsInfoForGeneration.map((info) => {
  ...
  ${propInfoForGeneration.map((element) => {
      if (element.format == "DateTime") {
        return (
          element.name +
          ":" +
          `${element.name}${pageNameSingular}Prop ? new Date(${element.name}${pageNameSingular}Prop): new Date()`
        );

      } else {
        return element.name + ":" + element.name + `${pageNameSingular}Prop`;
      }
    }
    ...
    return ` ${info.DisplayName}${info.EntityName}: ${info.DisplayName}${info.EntityName}Prop`
  }).join(",")}
  });
  ...
  useEffect(() => {
    ...
    .map((element, i) => {
      ...
      if (element.IsRequired) {
        return `&& data.${element.DisplayName}`
      } else {
        return ''
      }
    }).join(" ")} )
...
}, [${propInfoForGeneration.map((element) => element.name + `${pageNameSingular}Prop`)},${navPropInfoForGeneration.map((info) => {
      return `${info.DisplayName}${info.EntityName}Prop `
    })} ]);
...
${propInfoForGeneration
    ...
    .map((element) => {
      return `const selectHandler${element.DisplayName}${element.EntityName} = (e:any)=>{
    setData((prevData)=>({...prevData, ${element.DisplayName}${element.EntityName}:e.value}))
  }`;
    })
    .join("")}
...
const handleSave = () => {
  ...
  setData({${propInfoForGeneration
    ...
    .map((element) => {
      ...
      return `${element.DisplayName}${element.EntityName}: ""`;
    })
    .join(", ")}
});
...
{ ${property.showincreatemodal && property.showineditmodal ? "" : (property.showincreatemodal ? upperCreateCondition : (property.showineditmodal ? upperUpdateCondition : ""))}
  ...
  <RdsSelectList
    ...
    selectedValue={${property.name}${pageNameSingular}Prop}
  ></RdsSelectList>
  ${lowerHtml}
  }
  `;
...
id={"${info.EntityName}"}
label={"${info.EntityName}"}
placeholder={t("Select ${info.EntityName}") || ""}
selectItems={${info.DisplayName}${info.EntityName}ListProp}
onChange={selectHandler${info.DisplayName}${info.EntityName}}
selectedValue={${info.DisplayName}${info.EntityName}Prop}
></RdsSelectList>
${lowerHtml}
`;
  })}
...
const basicTableHeaders = dataToShowOnTable.map((tableProps) => {
  return {
    displayName: convertToDisplayCase(`${tableProps.name}`),
    actualKey: convertToCamelCase(`${tableProps.name}`),
    name: `${tableProps.name}`,
    key: convertToCamelCase(`${tableProps.name}${tableProps.format}`),
    datatype: "text",
    sortable: true,
...
const NavTableHeaders = navPropInfoForGeneration.map((tableProps) => {
  return {
    displayName: convertToDisplayCase(`${tableProps.EntityName}`),
    actualKey: convertToCamelCase(`${tableProps.DisplayName}`),
    key: convertToCamelCase(`${tableProps.EntityName}${tableProps.DisplayName}`),
    pascalCaseDisplayName: convertToPascalCase(`${tableProps.DisplayName}${tableProps.EntityName}`),
    datatype: "text",
    sortable: true,
...
const realNavHeader = NavTableHeaders.map((item) => {
  return {
...
const listTableHeaders = basicTableHeaders.concat(realNavHeader)

const tableheaders = listTableHeaders.map((prop) => {
  if (prop.element == "RdsCheckbox") {
    return {
...
${navPropInfoForGeneration.map((property) => {
      return `dispatch(get${sliceReducerName}${property.EntityName}LookupRequest({}) as any);`
    })}},[]);`
  ) : ""}
...
${navPropInfoForGeneration.length != 0 ? (
    `useEffect(()=>{
  ${navPropInfoForGeneration.map((property) => {
      return `if(JSON.stringify(data${sliceReducerName.toLowerCase()}.get${sliceReducerName}${property.EntityName}Lookup)!=="{}"){
const tempNavList = data${sliceReducerName.toLowerCase()}.get${sliceReducerName}${property.EntityName}Lookup?.items?.map((data:any)=>{
return {
...
set${property.DisplayName}List(tempNavList)
  ...
  ${navPropInfoForGeneration.map((property) => {
      return `data${sliceReducerName.toLowerCase()}.get${sliceReducerName}${property.EntityName}Lookup`
    })}
]);`
  ) : ""}
...
useEffect(() => {
  ...
  ?.items != null) {
    ...
    if (element.element == "RdsDatePicker") {
      return `${element.actualKey}:getDatefortable(item.${convertToCamelCase(`${pageNameSingular}`)}.${element.actualKey})`
    } else if (element.element == "RdsSelectList") {
      return `${element.actualKey}:  ${element.name}Enum[item.${convertToCamelCase(`${pageNameSingular}`)}.${element.actualKey}].option`
    } else if (element.element == "RdsCheckbox") {
      return `${element.actualKey}:item.${convertToCamelCase(`${pageNameSingular}`)}.${element.actualKey}.toString(),
             ${element.key}:(
              <>
                  {item.${convertToCamelCase(`${pageNameSingular}`)}.${element.actualKey} == true ? (
                      <div>
                          <RdsIcon
                              name="check"
                              height="17px"
      ...
      return `${element.actualKey}:item.${convertToCamelCase(`${pageNameSingular}`)}.${element.actualKey}`
    }

  })},    ${NavTableHeaders.map((data) => {
    return `${data.key}:item.${convertToCamelCase(`${data.entityName}`)}?.${data.actualKey},
    ${convertToCamelCase(`${data.entityName}`)}id:item.${convertToCamelCase(`${data.entityName}`)}?.id,
    `
  })}
          }
      });
...
return dataId.value 
    ...
    ${NavTableHeaders.map((element) => {
      return ` let ${element.key}Index = datafromcomponent.data.${element.pascalCaseDisplayName}
          `
    }).join(" ")}
    ...
    ${navPropInfoForGeneration.map((element) => {
      return `,${convertToCamelCase(`${element.propforcrud}`)} : ${convertToCamelCase(`${element.EntityName}${element.DisplayName}`)}Index.toString()`
    })}
  ...
  useEffect(() => {
    ...
    const onActionSelection = (rowData: any, actionId: any) => {
      ...
      if (element.element == "RdsSelectList") {
        return ` let ${element.name}Index = rowData.${element.actualKey}`
      }
    }).join(" ")}
      setdataEmit({
        ...
        ${basicTableHeaders.map((element) => {
      if (element.element == "RdsSelectList") {
        return `${element.actualKey}: ${element.name}Index`
      } else {
        return ` ${element.actualKey} : rowData.${element.actualKey}`
      }
...
<div className="container-fluid m-0 p-0">
                    <div className="row">
                            ...
                            <div className="card px-4 py-4 border-0 rounded-0 card-full-stretch">
                                <div className="mb-3 d-flex justify-content-end">
                                    ...
                                    >
                                      ...
                                      ${propInfoForGeneration.map((element) => {
                          if (element.format == "bool") {
                            return `${element.name}${pageNameSingular}Prop =  ${element.DefaultValue ? `{${element.DefaultValue}}` : "{false}"}`
                          } else if (element.format == "string") {
                            return `${element.name}${pageNameSingular}Prop = ${element.DefaultValue ? `{"${element.DefaultValue}"}` : `{""}`}`
                          } else if (element.format == "int") {
                            return `${element.name}${pageNameSingular}Prop = ${element.DefaultValue ? `{${element.DefaultValue}}` : `{""}`}`
                          }
                          else {
                            return `${element.name}${pageNameSingular}Prop = {""}`
                          }
                        }).join("\n")}
                        ${navConnectionsInfoForGeneration.map((info) => {
                        ...
                        ${navPropInfoForGeneration.map((element) => element.DisplayName + `${element.EntityName}Prop={""} \n  ${element.DisplayName}${element.EntityName}ListProp={${element.DisplayName}List}`).join("\n  ")}
                      ...
                      if (element.format == "bool") {
                        return `${element.name}${pageNameSingular}Prop = {dataEmit.${convertToCamelCase(`${element.name}`)} == "true" ?true:false}`
                      } else {
                        return `${element.name}${pageNameSingular}Prop = {dataEmit.${convertToCamelCase(`${element.name}`)}}`
                      }
                    }).join("\n")}
                    ...
                    ${navPropInfoForGeneration.map((element) => element.DisplayName + `${element.EntityName}Prop={dataEmit.${convertToCamelCase(`${element.EntityName}${element.DisplayName}`)}} \n  ${element.DisplayName}${element.EntityName}ListProp={${element.DisplayName}List}`).join("\n  ")}
