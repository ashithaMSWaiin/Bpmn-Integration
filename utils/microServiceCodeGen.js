...
const OpenAPI = require('bpmn-integration-react-core/dist/build-proxy');
...
const generateRealWorldSpecs = async () => {
    ...
    await generate(list, `./bpmn-integration-mfe/libs/${eTc}/${proxyGeneratedFileName}`);

    // Replacing the API URL in the .env file
    const envConfig = path.resolve(
        __dirname, '../', 'bpmn-integration-mfe', 'rds_pages', 'host', '.env'
    );
    ...
    const OpenAPIConfig = path.resolve(
        __dirname, '../', 'bpmn-integration-mfe', 'libs', 'proxy', `${proxyGeneratedFileName}`, 'core', 'OpenAPI.ts'
    );
    ...
    const MainTSX = path.resolve(
        __dirname, '../', 'bpmn-integration-mfe', 'rds_pages', 'host', `src`, 'Main.tsx'
    );
    ...
    const interceptor = path.resolve(
        __dirname, '../', 'bpmn-integration-mfe', 'libs', 'shared', 'interceptor.ts'
    );
    ...
    const NewProxyIndex = path.resolve( __dirname, '../', 'bpmn-integration-mfe', 'libs', 'proxy',`${proxyGeneratedFileName}`, 'index.ts'
    );
    ...
    const OpenAPIIndex = path.resolve(
        __dirname, '../', 'bpmn-integration-mfe', 'libs', 'proxy', 'index.ts'
    );
