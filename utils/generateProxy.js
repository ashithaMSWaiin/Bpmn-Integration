...
const generateRealWorldSpecs = async () => {
    ...
    await generate(list, `./bpmn-integration-mfe/libs/${eTc}`);
    ...
    const OpenAPIConfig = path.resolve(
        __dirname, "../", "bpmn-integration-mfe", "libs", "proxy", "core", "OpenAPI.ts"
    );
    ...
    const PermissionContentpath = path.resolve(
        __dirname, "../", "bpmn-integration-mfe", "rds_pages", "host", "src", "PrivateRoute_Auth.tsx"
    );
    //
    const MainMenuDashboardPath = path.resolve(
        __dirname, "../", "bpmn-integration-mfe", "libs", "main-menu", "main-menu.ts"
    );
    ...
    const LoginTSX = path.resolve(
        __dirname, "../", "bpmn-integration-mfe", "rds_pages", "rds-page-login", "src", "Login", "Login.tsx"
    );
