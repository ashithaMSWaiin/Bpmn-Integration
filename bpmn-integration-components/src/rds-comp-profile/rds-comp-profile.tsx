...
const RdsCompProfile = (props: RdsCompProfileProps) => {
    ...
    useEffect(() => {
        if (window.location.pathname !== "/my-account" && window.location.pathname !== "/security-Logs" 
        && window.location.pathname !== "/personal-data" &&  window.location.pathname !== "/linked-accounts"
        &&  window.location.pathname !== "/authority-delegation" &&  window.location.pathname !== "/impersonation") {
            setAcivetab("");
        }
    ...
    const onSetNavTabHandler: any = (id: any, navigateTo: string) => {
        ...
        console.log("id navtab id - ", id + " navigateTo - ", navigateTo    );
        setAcivetab(id);
                        ...
                        {props.navtabItems.map((item: any, i) => (
                            <div key={i} data-bs-dismiss="offcanvas">
                                ...
                                >
                                    ...
                                    <div>
                                        <div
                                            className={`fw-semibold text-capitalize active-profile ${activetab == item.id ? " " : ""
                                                }`}
                                        >
                                            {item.label}
                                        </div>
                                        <p className="text-break m-0 active-profile">{item.subText}</p>
                                    </div>
                                </li>
                                <RdsOffcanvas
                                    offId={item.id}
