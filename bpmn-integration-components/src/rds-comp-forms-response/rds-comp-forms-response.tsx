...
const RdsCompFormsResponse = (props: RdsCompFormsResponseProps) => {
  ...
  function compareQuestions(a: any, b: any) {
    ...
    // Extract numeric values from titles or default to 0 if no digits found
    const questionNumberA = parseInt(
      (a.title.match(numberPattern) || ["0"])?.[0]
    );
    const questionNumberB = parseInt(
      (b.title.match(numberPattern) || ["0"])?.[0]
    );

    // If both numeric values are valid (not NaN), compare them
    if (!isNaN(questionNumberA) && !isNaN(questionNumberB)) {
      return questionNumberA - questionNumberB;
    }

    // If one or both numeric values are NaN, compare titles alphabetically
